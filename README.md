...
- 2026-07-14：已实机确认同步 hooks 持续写入活动文件，最近任务按独立 Codex 会话展示。为避免同一项目的多个无标题会话看起来重复，展示层补充仅由既有会话标识派生的短任务号；不记录提示词或回复内容。
- 2026-07-14：收敛为“额度快照 + Codex 原生 hooks”的轻量组合：悬浮球直接显示可切换的 5 小时/7 天剩余百分比，主界面改为 Codex 浅色主题（`#FFFFFF` / `#1A1C1F` / `#339CFF`）。活动事件只更新悬浮层，不再重建账号页以消除高频闪烁；安装器改为验证四个正式 hook 已落盘，而不是依赖窗口 EXE 不可靠的退出码。本机已验证正式 EXE 写入 JSONL、四个 hook 无 `async` 并指向安装目录。
- 2026-07-13：按 Codex 近黑底+绿色焦点方向完成第二轮 UI 打磨：统一高对比调色盘、加粗中文可读字体层级、悬浮球图标改为超采样抗锯齿、托盘增加“置顶/取消置顶悬浮球”，并在活动事件陈旧或缺失时隐藏旧任务、显示“修复状态连接”入口，避免最近任务误报。
